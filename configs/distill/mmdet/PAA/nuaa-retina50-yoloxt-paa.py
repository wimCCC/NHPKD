...
model = dict(
    ...
    distiller=dict(
        distill_losses=dict(
            loss_pkd_fpn0=dict(loss_weight=1, type='PAALoss'),
            loss_pkd_fpn1=dict(loss_weight=1, type='PAALoss'),
            loss_pkd_fpn2=dict(loss_weight=1, type='PAALoss')),
    ...
    teacher_ckpt = 'checkpoint/teacher/retina50_epoch_280.pth',
    type='FpnTeacherDistill')
optim_wrapper = dict(
    _scope_='mmdet',
    clip_grad=dict(max_norm=2, norm_type=2),
    optimizer=dict(lr=0.03, momentum=0.9, type='SGD', weight_decay=0.001),
    type='OptimWrapper')
param_scheduler = [
    dict(
        _scope_='mmdet',
...
work_dir = './PAA/nuaa-retina50-yoloxt-PAA'
